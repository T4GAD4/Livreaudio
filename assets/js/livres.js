...
function AfficheLivre(livre) {
    ...
    var audio = "<audio controls data-idlivre='"+livre[0].idLivre+"'>\n\
...
function SaveTime() {
    if ($('audio').length != 0) {
        ...
        data.time = time;
        data.idLivre = $('audio')[0].dataset.idlivre;
...
function livres(id) {
    $("#content").load("vues/livre.php");
    loadLivre(id);
}

function AfficheLivres(data) {
    SaveTime();
    var titre = 'L\'ensemble de nos livres audio<br/><br/>\n\
<input type="text" class="form-control" id="research" placeholder="Rechercher..." onkeyup="research()">';
    $('#titre').html(titre);
    data = JSON.parse(data);
    var livres = "";
